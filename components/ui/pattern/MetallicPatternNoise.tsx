    ...
    <div style={styles as CSSProperties} className={`grid place-items-center w-full ${className}`.trim()}>
      <svg className="absolute w-px h-px -m-px">
        <defs>
          <filter id="noise">
            ...
            <feTurbulence result="waves" type="turbulence" baseFrequency="1.2 1.3" numOctaves="2" seed="256"></feTurbulence>
            <feDisplacementMap in="blur" in2="waves" scale="80" xChannelSelector="R" yChannelSelector="B" result="ripples"></feDisplacementMap>
            <feComposite in="waves" in2="ripples" operator="arithmetic" k1="1.2" k2="0" k3="1" k4="0"></feComposite>
          </filter>
        </defs>
      </svg>
      ...
      >
        ...
        <div 
          className="absolute inset-0"
          style={{
            background: 'radial-gradient(circle at 100% 100%, var(--fg) 15%, 40%, var(--fg-alpha) 70%)'
          }}
        />
      </div>
    </div>
