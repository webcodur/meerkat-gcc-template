  ...
  const handleGroupMove = (direction: GroupMoveDirection) => {
    switch (direction) {
      case 'first':
        ...
        break
      case 'prev':
        const previousGroupFirstPage = Math.max(((activeButtonGroupIndex - 2) * VISIBLE_BUTTON_COUNT) + 1, 1)
        setActivePageNumber(previousGroupFirstPage)
        break
      case 'next':
        const nextGroupFirstPage = Math.min(activeButtonGroupIndex * VISIBLE_BUTTON_COUNT + 1, totalPageCount)
